...
welcome(Req, State) ->
	...
	% Url_more_news = showbiz_util:more_news(Category, integer_to_list(?LATEST_NEWS_PER_PAGE), integer_to_list(SkipItems)), 
	% % io:format("params ~p ~n ",[Url_more_news]),	
	...
	%io:format("params ~p ~n ",[ParamsAllNews]),
	 ...
	 % for video display
	% Url = showbiz_util:video_count(binary_to_list(<<"full_composite_article">>),binary_to_list(<<"1">>),binary_to_list(<<"2">>)),
